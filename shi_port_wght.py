...
RISK_FREE_RATE = 0.02  # risikofreier Zinssatz p.a.
BUSINESS_DAYS = 'B'

# ---- Helper Functions ----
def to_1d_series(series: pd.Series | pd.DataFrame) -> pd.Series:
    """Konvertiert DataFrame-Spalte oder Series in eine 1D-NumPy-gefilterte Serie."""
    if isinstance(series, pd.DataFrame):
        series = series.iloc[:, 0]
    return pd.to_numeric(series, errors='coerce').dropna()

def omega_ratio(returns: pd.Series, risk_free: float = 0.0) -> float:
...
def fetch_data(tickers: list[str], start: datetime, end: datetime) -> tuple[dict[str, pd.Series], dict[str, pd.Series], dict[str, pd.Series]]:
    """
    Lädt Schlusskurse von Yahoo, füllt Business-Day-Index vorwärts auf, berechnet Renditen und kumulierte Performance.
    """
...
def compute_metrics(returns: pd.Series, cumulative: pd.Series) -> dict[str, float]:
    """
    Berechnet Risikokennzahlen für eine gegebene Rendite- und kumulierte Serie.
    """
    ...
    ann_ret = float((1 + total)**(365 / days) - 1) if days > 0 else np.nan
    ann_vol = float(r.std() * np.sqrt(252))
    stats['Total Return'] = total
    stats['Annual Return'] = ann_ret
    stats['Annual Volatility'] = ann_vol
    stats['Sharpe Ratio'] = ((ann_ret - RISK_FREE_RATE) / ann_vol) if ann_vol > 0 else np.nan
    # Sortino
    downside = r[r < 0]
    ...
    # Drawdowns
    dd = c / c.cummax() - 1
    stats['Max Drawdown'] = float(dd.min())
    stats['Calmar Ratio'] = float(ann_ret / abs(stats['Max Drawdown'])) if stats['Max Drawdown'] < 0 else np.nan
    # VaR & CVaR
    var95 = float(r.quantile(0.05))
    stats['VaR (95%)'] = var95
    stats['CVaR (95%)'] = float(r[r <= var95].mean())
    # Omega & Tail
    stats['Omega Ratio'] = omega_ratio(r)
    stats['Tail Ratio'] = tail_ratio(r)
    # Win/Loss
...
def plot_line(data: dict[str, pd.Series], title: str, ylabel: str) -> None:
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        tickers = [t.strip().upper() for t in st.text_area('Tickers (kommagetrennt)', 'AAPL, MSFT').split(',') if t.strip()]
    if len(tickers) < 1:
    ...
    # Sharpe/Korrelation
    with tabs[2]:
        rolling = pd.DataFrame({t: returns[t].rolling(126).apply(lambda x: ((x.mean()*252 - RISK_FREE_RATE) / (x.std()*np.sqrt(252))) if x.std()>0 else np.nan) for t in tickers})
        plot_line(rolling, 'Rollierender Sharpe (126 Tage)', 'Sharpe')
    ...
    with tabs[4]:
        if len(tickers) < 2:
            ...
            sol = opt.minimize(
                lambda w: -((dfR.mean()*w).sum()*252 - RISK_FREE_RATE) / np.sqrt(w @ (dfR.cov()*252) @ w),
